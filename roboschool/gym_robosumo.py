...
class RoboschoolRoboSumo(SharedMemoryClientEnv):
    ...
    def calc_state(self):
        '''
        TODO: compute an observation state similar to the one used in RoboSumo
        '''
        ...
        if self.initial_z==None:
            self.initial_z = z
        self.sumo_target_theta = np.arctan2( self.sumo_target_y - self.body_xyz[1], self.sumo_target_x - self.body_xyz[0] )
        self.sumo_target_dist  = np.linalg.norm( [self.sumo_target_y - self.body_xyz[1], self.sumo_target_x - self.body_xyz[0]] )
        self.angle_to_target = self.sumo_target_theta - yaw
    ...
    def calc_potential(self):
        '''
        TODO : decide what type of reward to use...
        '''
        # progress in potential field is speed*dt, typical speed is about 2-3 meter per second, this potential will change 2-3 per frame (not per second),
        # all rewards have rew/frame units and close to 1.0
        return - self.sumo_target_dist / self.scene.dt
